...
env READARR_BOOKS_LXC=$fake_lxc READARR_BOOKS_TEST_LOG=$log READARR_BOOKS_TEST_STATE=$state READARR_BOOKS_TEST_DEVICES=$devices READARR_BOOKS_TEST_EXECUTABLE=$executable fish --no-config "$script_dir/verify-readarr-books.fish"
or begin; printf '%s\n' 'approved fake configuration did not verify' >&2; exit 1; end

if env READARR_BOOKS_LXC=$fake_lxc READARR_BOOKS_TEST_LOG=$log READARR_BOOKS_TEST_STATE=$state READARR_BOOKS_TEST_DEVICES=$devices READARR_BOOKS_TEST_EXECUTABLE=$executable READARR_BOOKS_TEST_BAD_AUDIO=1 fish --no-config "$script_dir/verify-readarr-books.fish" >/dev/null 2>&1
    printf '%s\n' 'verifier accepted an Audio mount' >&2
    exit 1
end

if env READARR_BOOKS_LXC=$fake_lxc READARR_BOOKS_TEST_LOG=$log READARR_BOOKS_TEST_STATE=$state READARR_BOOKS_TEST_DEVICES=$devices READARR_BOOKS_TEST_EXECUTABLE=$executable READARR_BOOKS_TEST_EXTRA_DISK=1 fish --no-config "$script_dir/verify-readarr-books.fish" >/dev/null 2>&1
...
for unsafe_state in READARR_BOOKS_TEST_BAD_ROOT READARR_BOOKS_TEST_BAD_BRIDGE READARR_BOOKS_TEST_BAD_IP READARR_BOOKS_TEST_PROXY
    if env READARR_BOOKS_LXC=$fake_lxc READARR_BOOKS_TEST_LOG=$log READARR_BOOKS_TEST_STATE=$state READARR_BOOKS_TEST_DEVICES=$devices READARR_BOOKS_TEST_EXECUTABLE=$executable $unsafe_state=1 fish --no-config "$script_dir/verify-readarr-books.fish" >/dev/null 2>&1
        printf 'verifier accepted unsafe state: %s\n' $unsafe_state >&2
        exit 1
    end
end

for unsafe_topology in READARR_BOOKS_TEST_BAD_AUDIO READARR_BOOKS_TEST_EXTRA_DISK READARR_BOOKS_TEST_PROXY READARR_BOOKS_TEST_RAW_IDMAP
    set -l enables_before (count (string match -r '^exec readarr-books -- systemctl enable --now readarr-books.service$' < $log))
    if env READARR_BOOKS_LXC=$fake_lxc READARR_BOOKS_TEST_LOG=$log READARR_BOOKS_TEST_STATE=$state READARR_BOOKS_TEST_DEVICES=$devices READARR_BOOKS_TEST_EXECUTABLE=$executable READARR_BOOKS_BUNDLE=$bundle $unsafe_topology=1 fish --no-config "$script_dir/provision-readarr-books.fish" >/dev/null 2>&1
        printf 'provisioning accepted unsafe topology: %s\n' $unsafe_topology >&2
        exit 1
    end
    test (count (string match -r '^exec readarr-books -- systemctl enable --now readarr-books.service$' < $log)) -eq $enables_before
    or begin; printf 'provisioning enabled service under unsafe topology: %s\n' $unsafe_topology >&2; exit 1; end
end
